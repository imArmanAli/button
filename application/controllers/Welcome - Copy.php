...
// 			$getcountry = $xml->geoplugin_countryName ;
	...
	public function popup_diff(){
        ...
        if (isset($user) && $user > 0 && isset($file) && $file != '') {
			...
			$_SESSION['buttonValue'] = $buttonValue + 1;
			$new_hash 	= time()+60;
			...
			//$userip = '5.107.23.82';
// 			$xml = simplexml_load_file("http://www.geoplugin.net/xml.gp?ip=".$userip);
// 			$getcountry = (string) $xml->geoplugin_countryName[0];
// 			$state = (string) $xml->geoplugin_regionName[0];
			$getcountry = 'Pakistan';
			$getcountry = 'Punjab';
			$cappingResult = $this->HomeModel->get_capping_details('tbl_capping_poplink', $user, $sid, $opver, $getcountry, $state);
// 			print_r($cappingResult);exit;
			if(empty($cappingResult)){
			    $cappingResult = new stdClass();
                $cappingResult->link1 = '';
                $cappingResult->link2 = '';
                $cappingResult->link3 = '';
                $cappingResult->link4 = '';
...
// 			print_r($cappingResult);exit;
			...
			// $qry = 'select * from tbl_capping_poplink where pub_id = ' . $user . ' AND site_id = ' . $sid . ' AND os = "' . $opver . '" AND country ="' . $getcountry . '" AND states like "%' . $city . '%" ';

			// $cappingResult = $this->db->query($qry);
			// print_r($cappingResult);exit;
				// $redirectUrl = $cappingResult->link1;
				// $redirectUrl = $cappingResult->link2;
				// $redirectUrl = $cappingResult->link3;
				// $redirectUrl = $cappingResult->link4;
				// $redirectUrl = $cappingResult->link5;
			...
			$successResponse = $redirecturl.'&template=popup'.'$type='.$type;
			...
			$jsCode = '(function () {
						let link = [];
						link.push("'.$cappingResult->link1.'");
						link.push("'.$cappingResult->link2.'");
						link.push("'.$cappingResult->link3.'");
						link.push("'.$cappingResult->link4.'");
						link.push("'.$cappingResult->link5.'");
					';
            $jsCode .= 'let navbar = document.querySelector("body");
						const input = document.createElement("input");
						input.setAttribute("type", "hidden");
						input.setAttribute("id", "clickButton");
						input.setAttribute("value", "0");
						navbar.appendChild(input);

						let getValue = document.getElementById("clickButton").value;
						
							
                        	document.addEventListener("click", function() {
								let url = link[getValue];
								if(getValue < 5 && url != ""){
									getValue = parseInt(getValue) +1;
									document.getElementById("clickButton").value = getValue;
	...
	public function popup_inline_diff(){
	     header('Access-Control-Allow-Origin: *');
        header('Content-Type: text/javascript');
        if(empty($_GET)){
            echo "Some error occured!!";
...
// 			$state = (string) $xml->geoplugin_regionName[0];
			$getcountry = 'Pakistan';
			$state = 'Punjab';
// 			$state = (string) $xml->geoplugin_city[0];
			
			
			
			$cappingResult = $this->HomeModel->get_capping_details('tbl_capping_pop_inline', $user, $sid, $os, $getcountry, $state);
...
<body class="notranslate" id="body">
	<div class="_1iX3k _14UoU" contrast="dark" id="main">
		<div class="_3jqVQ">
			<div class="_15iRN"><img class="_119Er r5Jpz" src="'.R.'"></div>
			...
			<input type="hidden" id="clickButton" value="0">
			<div class="_3MkcZ r5Jpz">
				'.(($cappingResult->link1 != "")?"
				<a href=\"$cappingResult->link1\" id=\"div1\" target=\"_blank\" class=\"_1e8rA buttonPress-$user\" onclick=\"document.getElementById(\'div1\').style.display = \'none\';document.getElementById(\'div2\').style.display = \'block\';\">Open</a>
				<a href=\"$cappingResult->link1\" id=\"file1\" target=\"_blank\" class=\"_98yaB buttonPress-$user\" onclick=\"document.getElementById(\'file1\').style.display = \'none\';document.getElementById(\'file2\').style.display = \'block\';\">See File</a>
				":'').'
				
				'.(($cappingResult->link2 != "")?"
				<a href=\"$cappingResult->link2\" style=\"display:none\" id=\"div2\" target=\"_blank\" class=\"_1e8rA buttonPress-$user\" onclick=\"document.getElementById(\'div2\').style.display = \'none\';document.getElementById(\'div3\').style.display = \'block\';\">Open</a>
				<a href=\"$cappingResult->link2\" style=\"display:none\" id=\"file2\" target=\"_blank\" class=\"_98yaB buttonPress-$user\" onclick=\"document.getElementById(\'file2\').style.display = \'none\';document.getElementById(\'file3\').style.display = \'block\';\">See File</a>
				":'').'

				'.(($cappingResult->link3 != "")?"
				<a href=\"$cappingResult->link3\" style=\"display:none\" id=\"div3\" target=\"_blank\" class=\"_1e8rA buttonPress-$user\" onclick=\"document.getElementById(\'div3\').style.display = \'none\';document.getElementById(\'div4\').style.display = \'block\';\">Open</a>
				<a href=\"$cappingResult->link3\" style=\"display:none\" id=\"file3\" target=\"_blank\" class=\"_98yaB buttonPress-$user\" onclick=\"document.getElementById(\'file3\').style.display = \'none\';document.getElementById(\'file4\').style.display = \'block\';\">See File</a>
				":'').'

				'.(($cappingResult->link4 != "")?"
				<a href=\"$cappingResult->link4\" style=\"display:none\" id=\"div4\" target=\"_blank\" class=\"_1e8rA buttonPress-$user\" onclick=\"document.getElementById(\'div4\').style.display = \'none\';document.getElementById(\'div5\').style.display = \'block\';\">Open</a>
				<a href=\"$cappingResult->link4\" style=\"display:none\" id=\"file4\" target=\"_blank\" class=\"_98yaB buttonPress-$user\" onclick=\"document.getElementById(\'file4\').style.display = \'none\';document.getElementById(\'file5\').style.display = \'block\';\">See File</a>
				":'').'


				'.(($cappingResult->link5 != "")?"
				<a href=\"$cappingResult->link5\" style=\"display:none\" id=\"div5\" target=\"_blank\" class=\"_1e8rA buttonPress-$user\" onclick=\"document.getElementById(\'div5\').style.display = \'none\';document.getElementById(\'div1\').style.display = \'block\';\">Open</a>
				<a href=\"$cappingResult->link5\" style=\"display:none\" id=\"file5\" target=\"_blank\" class=\"_98yaB buttonPress-$user\" onclick=\"document.getElementById(\'file5\').style.display = \'none\';document.getElementById(\'file1\').style.display = \'block\';\">See File</a>
				":'').'
...
';
			$jsCode .= '<script type="text/javascript">
				...
				const buttonPress = document.getElementByClassNames("_1e8rA");
				...
				// 		getValue = parseInt(getValue) +1;
				// 		document.getElementById("clickButton").value = getValue;
				// 		//document.getElementByClassNames("buttonPress-'.$user.'").setAttribute("href", url);
				// 		return false;
				// 		window.open(url, \'_blank\'); return; }; for (var i = 0; i < elements.length; i++) { elements[i].addEventListener(\'click\', clickFunction, false);
				// 	}
				// }
			
				function () {

					$("._1e8rA").click(function(){
						alert();
					})
			...
			if ($user > 0) {
				$jsCode .=	'var id = '.$user.'; 
					...
					var elements = document.getElementsByClassName("buttonPress-'.$user.'");
					var elementsClose = document.getElementsByClassName("ANdv8"); 
				';
   ...
   public function popup_inline(){
	   ...
	   $jsCode = 'var newScript = document.createElement(\'script\');
		 ...
		 document.head.appendChild(newScript);';
			if($banner_type == '1'){
			    $jsCode .= '
		document.getElementsByTagName(\'body\')[0].insertAdjacentHTML(\'afterbegin\',\'<iframe id="iframe" style="width: 100% !important; height: 149px !important; margin: 0px; padding: 0px; border: none; outline: none; box-sizing: border-box;';
	
			$jsCode .= 'position: fixed; inset: auto 0px 0px auto !important;max-width: 400px !important;" overflow: hidden; z-index: 2147483640; min-width: 240px !important; display: block !important; "></iframe>\');';
		}else{
		    $jsCode .= '
			var popup_inline = document.querySelector("#popup_inline");
			var iframe = document.createElement("iframe");
			iframe.id = "iframe";
			iframe.style.cssText = "width: 100% !important; height: 149px !important; margin: 0px; padding: 0px; border: none; outline: none; box-sizing: border-box;max-width: 800px !important;overflow: hidden; z-index: 2147483640; min-width: 240px !important; display: block !important; ";
...
';
		...
		$jsCode .= '
		 setTimeout(function() {
		   ...
		   $user_esc = $this->db->escape($user);
		   $opver = $this->db->escape($opver);
		   $sql2 = "INSERT INTO tbl_statistics_btn(user_ip, stat_date, pub_id, domain_id,stat_os, stat_status)VALUES('$userip','$todaydate', $user_esc, '0', $opver, '0')";
		   $result2 = $this->db->query($sql2);
...
<body class="notranslate" id="body">
   <div class="_1iX3k _14UoU" contrast="dark" id="main">
	   <div class="_3jqVQ">
		   <div class="_15iRN"><img class="_119Er r5Jpz" src="'.$banner_image.'"></div>
		   <div class="_1mR5N r5Jpz">
			   <div class="_2i-v5">Download</div>
			   <div class="_2lRsN"></div>
		   </div>
...
';
			...
			<a class="_98yaB " style="text-decoration:none" href="'.$successResponse.'" target="_parent">
				<div class="banner">
					<div class="shopping-image">
						<img src="'.$banner_image.'" alt="shopping">
					</div>
					<div class="text">
						<h4>Free giveaway Premium Softwares</h4><br>
						<h5 class="_2lRsN"></h5>
					</div>
				</div>
		  	</a>
	   ...
	   if($banner_type == '1'){   
		  $jsCode .=     '`);
			...
			$jsCode .=     '`);
			   document.querySelector("#iframe").contentDocument.querySelector("._2lRsN").append(sitetitle);
			   var link = document.querySelector("#iframe").contentDocument.querySelector("._98yaB").getAttribute(\'href\');
			   var fresh_st = sitetitle.replace(/ /g,"+");
			   var st = fresh_st.replace(/[^a-zA-Z0-9 ]/g, "-");
			   var parsstring = link.replace("{KEYWORD}", st);
			   document.querySelector("#iframe").contentDocument.querySelector("._98yaB").setAttribute(\'href\',parsstring);
