...
class Welcome extends CI_Controller {
	...
	public function index(){
		...
		if (empty($this->input->get('file'))) {
            echo 'No direct script access allowed';exit;
        }
		//$uri = $_SERVER['REQUEST_URI'];
		...
		if($adcode == 1){
			...
			$full_parse_url = $explode_url[0].'welcome/'.$typeof .'?'.$simplifiedUrl.'&opver='.$opver.'&browser='.$browser.$file_var;
			echo file_get_contents($full_parse_url);
		}
		if($adcode == 2){
			$full_parse_url = $explode_url[0].'welcome/smartlink?'.$simplifiedUrl.'&opver='.$opver.'&browser='.$browser.$file_var;
			if($file_var == ''){
		...
		if($adcode == 3){
			$full_parse_url = $explode_url[0].'welcome/popup?'.$simplifiedUrl.'&opver='.$opver.'&browser='.$browser.$file_var;
			if($file_var == ''){
				echo 'something went wrong';
				exit;
			}
		...
		if($adcode == 4){
			$full_parse_url = $explode_url[0].'welcome/popup_inline?'.$simplifiedUrl.'&opver='.$opver.'&browser='.$browser.$file_var;
			echo file_get_contents($full_parse_url);
		}
	}
